...
namespace GraphProcessor
{
	...
	[Serializable]
	[CreateAssetMenu(fileName = "NodeGraph", menuName = "NodeGraphProcessor/Graph")]
	public class BaseGraph : ScriptableObject, ISerializationCallbackReceiver
	{
		...
		/// <summary>
		/// Reorders nodes by position (Y first, then X). Call from editor when node positions change.
		/// </summary>
		public void SortNodesByPosition()
		{
			nodes.Sort((a, b) =>
			{
				...
				return cmp != 0 ? cmp : a.position.x.CompareTo(b.position.x);
			});
		}
