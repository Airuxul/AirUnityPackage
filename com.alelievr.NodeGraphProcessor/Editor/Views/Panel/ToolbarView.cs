...
namespace GraphProcessor
{
	public class ToolbarView : VisualElement
	{
		...
		Dictionary< Type, ToolbarButtonData >	pinnedViewToggles = new Dictionary< Type, ToolbarButtonData >();
		...
		protected virtual void AddButtons()
		{
			...
			pinnedViewToggles.Clear();
			foreach (var entry in PinnedViewRegistry.Entries)
			{
				var viewType = entry.viewType;
				bool visible = graphView.GetPinnedElementStatus(viewType) != Status.Hidden;
				var toggle = AddToggle(entry.ToolbarLabel, visible, (v) => graphView.ToggleView(viewType));
				pinnedViewToggles[viewType] = toggle;
			}
		...
		{
			foreach (var kvp in pinnedViewToggles)
				kvp.Value.value = graphView.GetPinnedElementStatus(kvp.Key) != Status.Hidden;
		}
