...
namespace GraphProcessor
{
	...
	public class BaseGraphView : GraphView, IDisposable
	{
        ...
        GraphViewChange GraphViewChangedCallback(GraphViewChange changes)
		{
			...
			{
				schedule.Execute(UpdateEdgeExecutionOrderLabels).ExecuteLater(0);
			}
		...
		public override List< Port > GetCompatiblePorts(Port startPort, NodeAdapter nodeAdapter)
		{
			...
			compatiblePorts.AddRange(ports.ToList().Where(p => {
				...
				// Data flows from output to input. Check convertibility: outputType -> inputType.
				var outputType = startPort.direction == Direction.Output ? startPort.portType : p.portType;
				var inputType = startPort.direction == Direction.Output ? p.portType : startPort.portType;
		...
		{
			foreach (var entry in PinnedViewRegistry.Entries)
			{
				var viewType = entry.viewType;
				evt.menu.AppendAction(entry.MenuLabel, (e) => ToggleView(viewType), (e) => GetPinnedElementStatus(viewType));
			}
		}
		...
		internal void UpdateSerializedProperties()
		{
		...
		/// <summary>
		/// Update execution order labels: only for ports with [ExecutionOrder] attribute, multiple connections, sorted by input node Y.
		/// </summary>
		public void UpdateEdgeExecutionOrderLabels()
		{
			if (graph == null) return;

			foreach (var edgeView in edgeViews)
				edgeView.UpdateExecutionOrderLabel(null);

			foreach (var edgeView in edgeViews)
			{
				if (edgeView.output is not PortView outputPortView || edgeView.input is not PortView inputPortView)
					continue;
				if (!outputPortView.HasExecutionOrder) continue;

				var portEdges = outputPortView.GetEdges();
				if (portEdges.Count < 2) continue;

				var sorted = portEdges.OrderBy(e => BaseNode.GetExecutionOrderSortKey((e.input as PortView).owner.nodeTarget)).ToList();

				var idx = sorted.IndexOf(edgeView);
				if (idx >= 0) edgeView.UpdateExecutionOrderLabel(idx + 1);
			}
		}
		...
		public bool ConnectView(EdgeView e, bool autoDisconnectInputs = true)
		{
			...
			schedule.Execute(() => {
				e.UpdateEdgeControl();
				UpdateEdgeExecutionOrderLabels();
			}).ExecuteLater(1);
