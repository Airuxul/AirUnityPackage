...
com.air.unity-game-core        GameRuntime + EventBus + 资源 + 定时器 + UnityObjectPool
    ↑
com.air.ui                     UIFramework + UIManager + UIScopedEvents（可选）

独立：gameplay-tag | behavior-tree → node-graph-processor | timeline-kit
```
...
| `com.air.ui` | `UIFramework`、`UIManager`、`UIScopedEvents`、`GameEntry` | 实现 EventBus / 资源加载 |
